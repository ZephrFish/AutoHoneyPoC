...
def createGitRepo(cve, summary, GITHUB_USERNAME, GITHUB_TOKEN, PWD):
	...
	data = {
		'name': cve,
		'description': f'PoC for exploiting {cve} : {summary}'
		}
	r=requests.post('https://api.github.com/user/repos', json=data, auth=(GITHUB_USERNAME, GITHUB_TOKEN))
	os.system(f'git config user.name "User"')
	os.system(f'git config user.email "User@example.com"')
	os.system(f'git commit -m "{cve} Commit"')
	os.system('git branch -m main')
	os.system(f'git remote add origin https://{GITHUB_USERNAME}:{GITHUB_TOKEN}@github.com/{GITHUB_USERNAME}/{cve}.git')
